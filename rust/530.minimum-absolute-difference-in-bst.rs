...
pub fn get_minimum_difference_against_target(
    root: Option<Rc<RefCell<TreeNode>>>,
    target: i32,
    node_to_avoid: Rc<RefCell<TreeNode>>,
) -> Option<i32> {
    root.map(|root| -> Option<i32> {
        ...
        let left_abs_diff = root_borrow
            .left
            .clone()
            .map(|x| (x.borrow().val.clone() - target).abs());
        let right_abs_diff = root_borrow
            .right
            .clone()
            .map(|x| (x.borrow().val.clone() - target).abs());
        let values = vec![root_abs_diff, left_abs_diff, right_abs_diff];
        let indices = values
            .iter()
            .enumerate()
            .filter_map(|(i, x)| x.map(|_| i))
            .collect::<Vec<_>>();
        let min_value = values.iter().filter_map(|x| x.clone()).min();
        let directions_to_check = indices.into_iter().filter_map(|i| {
            min_value
                .map(|min_value| {
                    values[i].map(|val| match val == min_value {
                        true => Some(i),
                        false => None,
                    })
                })
        ...
        directions_to_check
            .filter_map(|x| match x {
                ...
                1 => left_abs_diff
                    .map(|left_abs_diff| {
                        get_minimum_difference_against_target(
                            root_borrow.left.clone(),
                            target,
                            node_to_avoid.clone(),
                        )
                        .map(|x| left_abs_diff.min(x))
                    })
                    .flatten(),
                2 => right_abs_diff
                    .map(|right_abs_diff| {
                        get_minimum_difference_against_target(
                            root_borrow.right.clone(),
                            target,
                            node_to_avoid.clone(),
                        )
                        .map(|x| right_abs_diff.min(x))
                    })
                    .flatten(),
...
pub fn get_minimum_difference_overall(
    ...
    match node {
        Some(node) => {
            ...
            let node_target_diff =
                get_minimum_difference_against_target(root.clone(), node_val,node.clone()).unwrap_or(i32::MAX);
            let left_val = get_minimum_difference_overall(borrowed_node.left.clone(), root.clone());
            let right_val =
                get_minimum_difference_overall(borrowed_node.right.clone(), root.clone());
            vec![node_target_diff, left_val, right_val]
                .into_iter()
                .min()
                .unwrap()
        }
